...
void GiMain(const uint3 id: SV_DispatchThreadID)
{
    ...
    const float3 positionW = Quaternion::Apply(vsToWsQuaternion, - SceneConstants.m_fullscreen.m_cameraTranslation);
    ...
    const float minDist = SceneConstants.m_fullscreen.m_cameraTranslation.y - 3;
    const float maxDist = SceneConstants.m_fullscreen.m_cameraTranslation.y + 3;
    ...
    if (depthSs != 1)
    {
        ...
        const float3 hitPositionV = Quaternion::Apply(SceneConstants.m_fullscreen.m_cameraQuaternion, hitPositionW) + SceneConstants.m_fullscreen.m_cameraTranslation;

        if (depthV < hitPositionW.y)
        {
            return;
        }
        color = lerp(color, GBuffer0.Load(int3(pixelCoordinates, 0)).rgb * GBuffer1.Load(int3(pixelCoordinates, 0)).rgb, 0.2);
    }
